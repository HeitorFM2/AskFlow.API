...
namespace AskFlow.Tests.Application.Likes.Validators
{
    public class GetLikedPostsQueryValidatorTests
    {
        private readonly GetLikedPostsQueryValidator _sut = new();

        [Fact]
        public void Validate_DefaultPaging_ShouldPass()
        {
            var result = _sut.Validate(new GetLikedPostsQuery());
        ...
        {
            var result = _sut.Validate(new GetLikedPostsQuery(page, pageSize));
            result.IsValid.Should().BeTrue();
        }
        ...
        {
            var result = _sut.Validate(new GetLikedPostsQuery(page, 20));
            result.IsValid.Should().BeFalse();
            result.Errors.Should().Contain(e => e.PropertyName == nameof(GetLikedPostsQuery.Page)
        ...
        {
            var result = _sut.Validate(new GetLikedPostsQuery(1, pageSize));
            result.IsValid.Should().BeFalse();
            result.Errors.Should().Contain(e => e.PropertyName == nameof(GetLikedPostsQuery.PageSize)
        ...
        {
            var result = _sut.Validate(new GetLikedPostsQuery(0, 0));
            result.IsValid.Should().BeFalse();
