...
namespace AskFlow.Tests.Application.Users.Handlers
{
    public class UpdateAvatarHandlerTests
    {
        ...
        private UpdateAvatarHandler CreateSut() => new(_userManager, _avatarStorage, _currentUser, _logger);
        ...
        public async Task Handle_HappyPath_ShouldDeleteOld_UploadNew_PersistUrl_ReturnSuccess()
        {
            ...
            _avatarStorage.UploadAsync(Arg.Any<Stream>(), "image/jpeg", "u1", Arg.Any<CancellationToken>())
                .Returns("https://blob/u1.jpg");
            _userManager.UpdateAsync(user).Returns(IdentityResult.Success);

            var result = await CreateSut().Handle(Command(), default);

            result.IsSuccess.Should().BeTrue();
            result.Value.Should().Be("https://blob/u1.jpg");
            user.AvatarUrl.Should().Be("https://blob/u1.jpg");
            Received.InOrder(async () =>
            {
                await _avatarStorage.DeleteAsync("u1", Arg.Any<CancellationToken>());
                await _avatarStorage.UploadAsync(Arg.Any<Stream>(), "image/jpeg", "u1", Arg.Any<CancellationToken>());
